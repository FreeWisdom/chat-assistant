...
class WeChatBot:
    ...
    def _init_groups(self):
        ...
        for group_name in self.cm.group_map:
            ...
            try:
                ...
                logger.info(f"  ✅ {group_name} → {course.name} (已有 {count} 条消息)")
            except Exception as e:
    ...
    def _poll(self):
        ...
        while self._running:
            ...
            for group_name in group_list:
                ...
                course = self.cm.get_course(group_name)
                if not course or self.cm.is_excluded(group_name):
                    continue
                ...
                try:
                    ...
                    for msg in messages:
                        ...
                        # 处理消息，成功才标记指纹
                        if self._handle_message(msg, group_name, course):
    ...
    def _handle_message(self, msg, group_topic: str, course: Course) -> bool:
        ...
        try:
            ...
            # 是否为提问
            result = analyze_message(content, course, self.cm.smart_detection)
            ...
            answer = self.rag.answer(result["question_text"], course, history)
            logger.info(f"    📝 RAG 生成回答: {answer[:60]}...")

            # 发送
            self._send_reply(answer, group_topic)
    ...
    def _send_reply(self, text: str, group_name: str):
        """发送回复，超长则分段"""
