...
机器人消息后继续提问。推荐路线是本项目负责微信群收发和安全边界，MaxKB 负责知识库、提示词和云端模型 API 调用。
...
│   │   │   ├── routers/knowledge.py #   云知识库配置与旧百炼文档生命周期
...
│   │   ├── knowledge/               # MaxKB/百炼适配、问题路由、联网搜索、LLM 生成
...
MAXKB_API_KEY=agent-your-maxkb-api-key
DRY_RUN=true
...
knowledgeBases:
  - id: my-kb
    name: 我的 MaxKB 应用
    provider: maxkb
    maxkbAppId: your-maxkb-application-id
bindings:
  - group: 项目研究
    botId: my-bot
...
通过管理页 `http://127.0.0.1:8000` 也可以图形化编辑。MaxKB 路线下，
云端模型 Key、知识库文档、切分和索引都在 MaxKB 控制台维护；本项目只保存
`MAXKB_BASE_URL`、`MAXKB_API_KEY` 和每个知识库绑定的 `maxkbAppId`。
这样本项目启动时不再强制要求 `LLM_API_KEY`，前提是监听群只绑定
`provider: maxkb` 的知识库。

旧的阿里云百炼 provider 仍保留文档列表、版本历史、替换和删除：
...
RAM 授权、业务空间成员关系和平台 Workspace 仅适用于旧百炼 provider。
...
MaxKB provider 会把问题直接交给绑定的 MaxKB 应用，由 MaxKB 内部完成知识库检索和云端模型回答，本项目不会再二次调用 DeepSeek。非 MaxKB provider 仍保留原来的本项目 LLM 路由、知识库检索、联网搜索和最终润色链路。
...
该命令会实际消耗当前搜索提供方、MaxKB 应用或本项目 LLM provider 的 API 额度，但不会读取或发送微信消息。
...
| 知识库 | MaxKB 应用 API 优先，百炼 provider 保留 | MaxKB 管理知识库和云端模型，本项目只调用应用并保留旧百炼兼容 |
