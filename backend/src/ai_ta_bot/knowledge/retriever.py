    ...
    def search(self, knowledge_base_ids: list[str], query: str, top_k: int) -> list[dict[str, Any]]:
        ...
        for doc in documents:
            ...
            if score > 0:
                ...
                item["_priority"] = float(doc.get("priority", 0) or 0)
                scored.append((score, item))

        scored.sort(
            key=lambda item: (
                item[0],
                float(item[1].get("_priority", 0) or 0),
            ),
            reverse=True,
        )
    ...
    def search(self, knowledge_base_ids: list[str], query: str, top_k: int) -> list[dict[str, Any]]:
        ...
        for kb_id in knowledge_base_ids:
            ...
            for content, metadata, distance in zip(documents, metadatas, distances):
                ...
                if numeric_distance > config.VECTOR_MAX_DISTANCE:
                    continue
                doc = _doc_from_chroma(content, metadata)
                doc["_vector_distance"] = numeric_distance
